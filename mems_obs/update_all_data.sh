...
do
     ...
     do
         ...
         do
            CUDA_VISIBLE_DEVICES=1 nohup python -u mems_obs/update_data.py --name ${ENV}-${TYPE}-v0 --num_memories_frac ${F} --sum_rewards ${sum} > mems_obs/logs/${ENV}-${TYPE}-v0/update_data_${F}_frac_percentbc.log &
         done 
     done
 done
