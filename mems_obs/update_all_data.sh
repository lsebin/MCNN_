mkdir mems_obs/logs_re
 ...
 do
     ...
     do
         ...
         do
            CUDA_VISIBLE_DEVICES=0 nohup python -u mems_obs/update_data.py --name ${ENV}-${TYPE}-v2 --num_memories_frac ${F} > mems_obs/logs_re/${ENV}-${TYPE}-v2/update_data_${F}_frac_percentbc.log &
         done 
     done
 done
